...
export default function Character({ level, currentInteractionGif }: LevelProps) {
  const getCharacterSvg = () => {
    ...
    if (currentInteractionGif) {
      return <img src={currentInteractionGif} className="w-24 h-28" alt="Interaction GIF" />;
    }

    // 기본 캐릭터 표시
    if (level === 1) {
      return <img src={Babysomi.src} className="w-24 h-28" alt="Level 1 Character" />;
    }
    if (level === 2) {
      return <img src={Lv2.src} className="w-24 h-28" alt="Level 2 Character" />;
    }
    if (level === 3) {
      return <Level3 />;
    }
