...
export const reportMessageStyles = (count: number) => {
  const countColor = count > 7 ? 'text-green-500'
    : count > 5 ? 'text-blue-500'
    : count > 3 ? 'text-yellow-500'
    : 'text-gray-500';
  
  const messageColor = count > 7
    ? 'text-green-800'
    : count > 5
    ? 'text-blue-800'
    : count > 3
  ...
  const message = (
    <>
      {count > 3 ? (
        ...
        <p>공황 발작 대처를 위해 꾸준한 훈련이 중요합니다. <br/> 앞으로 함께 연습해볼까요?</p>
      )}
    </>
  );
